...
class OptunaTuner:
    ...
    def __init__(self, cfg: Dict = None):
        ...
        self.study = None
        self.n_trials = self.cfg.get('optuna_trials', 30)
        self.n_epochs = self.cfg.get('optuna_epochs', 20)

    def tune(self, X_train: np.ndarray, y_train_dict: Dict,
             X_val: np.ndarray, y_val_dict: Dict) -> Dict:
        
        if not OPTUNA_AVAILABLE:
            print("Optuna not available — using config defaults.")
            return self._defaults()
        
        if X_train.shape[0] < 50 or X_val.shape[0] < 50:
            print(f"Insufficient data: train={X_train.shape[0]}, val={X_val.shape[0]}. Using defaults.")
            return self._defaults()
        
        print(f"Starting {self.n_trials} Optuna trials with {self.n_epochs} epochs each...")
        
        def objective(trial):
            lu1 = trial.suggest_categorical('lstm_units_1', [64, 128, 192, 256])
            lu2 = trial.suggest_categorical('lstm_units_2', [32, 64, 96, 128])
            dr = trial.suggest_float('dropout_rate', 0.1, 0.5, step=0.05)
            lr = trial.suggest_float('learning_rate', 1e-5, 5e-3, log=True)
            ah = trial.suggest_categorical('attention_heads', [4, 8, 12])
            akd = trial.suggest_categorical('attention_key_dim', [32, 64, 96])
        ...
        try:
            ...
            if self.study.best_value > 10:
                print(f"Best loss {self.study.best_value:.4f} seems high, using defaults")
                return self._defaults()
    ...
    def _defaults(self) -> Dict:
        return {
            ...
            'attention_key_dim': self.cfg.get('attention_key_dim', 64),
