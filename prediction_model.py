...
class PredictionModel:
    ...
    NO_TRADE_ENTRY_QUALITY = 0.35
    NO_TRADE_ADX_MIN = 15.0
    NO_TRADE_VOL_SPIKE = 3.0
    ...
    def build(self, input_shape: Tuple[int, int]) -> Model:
        ...
        x = LSTM(self.cfg['lstm_units_1'], return_sequences=True, name='lstm_1')(x)
        x = Dropout(dr, name='drop1')(x)
        x = LSTM(self.cfg['lstm_units_2'], return_sequences=True, name='lstm_2')(x)
        x = Dropout(dr, name='drop2')(x)
        
        x = self._transformer_block(x, self.cfg['attention_heads'], self.cfg['attention_key_dim'], 256, dr, 't1')
        x = self._transformer_block(x, self.cfg['attention_heads']//2, self.cfg['attention_key_dim'], 128, dr, 't2')
        ...
        model.compile(
            optimizer=Adam(learning_rate=self.cfg['learning_rate'], clipnorm=1.0),
            loss={
                'price_pred': 'huber',
    ...
    @staticmethod
    def _engineer_targets(df: pd.DataFrame, close_scaled: np.ndarray, close_idx: int, max_exit_bars: int = 10) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        n = len(df)
    ...
    def prepare_data(self, df: pd.DataFrame, scaler=None, feature_cols: List[str] = None):
        exclude = {'timestamp'}
        ...
        if scaler is None:
            ...
            scaled = self.scaler.fit_transform(data).astype(np.float32)
        else:
            self.scaler = scaler
            scaled = scaler.transform(data).astype(np.float32)

        close_scaled = scaled[:, close_idx]
        y_price, y_dir, y_eq, y_exit, y_pos = self._engineer_targets(df, close_scaled, close_idx)

        window = self.cfg['window']
        X = np.array([scaled[i-window:i] for i in range(window, len(scaled)-1)], dtype=np.float32)

        y_price = y_price[window-1:].astype(np.float32)
        y_dir = y_dir[window-1:].astype(np.int32)
        y_eq = y_eq[window-1:].astype(np.float32)
        y_exit = y_exit[window-1:].astype(np.float32)
        y_pos = y_pos[window-1:].astype(np.float32)
        ...
        split = int(min_len * self.cfg.get('train_split', 0.8))
        self._feature_cols = feature_cols
        self._close_idx = close_idx

        print(f"Sequences: {X.shape}, Features: {X.shape[2]}, Train/Val: {split}/{min_len-split}")
        print(f"Direction distribution: Sell={np.sum(y_dir==0)}, Hold={np.sum(y_dir==1)}, Buy={np.sum(y_dir==2)}")

        return (
            X[:split], X[split:],
            {'price_pred': y_price[:split], 'direction': y_dir[:split], 'entry_quality': y_eq[:split], 'exit_bar': y_exit[:split], 'position_size': y_pos[:split]},
            {'price_pred': y_price[split:], 'direction': y_dir[split:], 'entry_quality': y_eq[split:], 'exit_bar': y_exit[split:], 'position_size': y_pos[split:]},
            feature_cols, close_idx
        )
    ...
    def train(self, X_train, X_val, y_train, y_val):
        ...
        print(f"Training complete. Conformal calibration size: {len(self._calib_price)}")

    def save(self, path=None):
        save_path = path or self.cfg.get('model_save_path', 'hybrid_v4_ultra.keras')
        self.model.save(save_path)
        if self.scaler:
            joblib.dump(self.scaler, self.cfg.get('scaler_save_path', 'hybrid_v4_scaler.pkl'))
        calib = {'price': self._calib_price.tolist(), 'quality': self._calib_quality.tolist(), 'exit': self._calib_exit.tolist()}
        with open(save_path.replace('.keras', '_calib.json'), 'w') as f:
            json.dump(calib, f)
        print(f"Model saved to {save_path}")

    def load(self, path=None):
        lp = path or self.cfg.get('model_save_path', 'hybrid_v4_ultra.keras')
        self.model = tf.keras.models.load_model(lp)
        self.scaler = joblib.load(self.cfg.get('scaler_save_path', 'hybrid_v4_scaler.pkl'))
        cp = lp.replace('.keras', '_calib.json')
