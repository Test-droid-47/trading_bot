...
class PPOAgent:
    ...
    def __init__(self, cfg: Dict = None, state_shape: Tuple = None, n_actions: int = 3):
        self.cfg = cfg or {}
    ...
    def _build_actor(self) -> Model:
        ...
        model = Model(inp, out, name='PPO_Actor')
        return model
    ...
    def _build_critic(self) -> Model:
        ...
        model = Model(inp, out, name='PPO_Critic')
        return model
    ...
    def collect_trajectory(self, env) -> Dict[str, np.ndarray]:
        state = env.reset()
    ...
    def update(self, states: np.ndarray, actions: np.ndarray, old_log_probs: np.ndarray,
        ...
        s_t = tf.constant(states, tf.float32)
        a_t = tf.constant(actions, tf.int32)
        lp_t = tf.constant(old_log_probs, tf.float32)
        r_t = tf.constant(returns, tf.float32)
        adv_t = tf.constant(advantages, tf.float32)
        ...
        for _ in range(self.ppo_epochs):
            with tf.GradientTape() as at:
                ...
                indices = tf.stack([tf.range(batch_size), a_t], axis=1)
                taken_probs = tf.gather_nd(probs, indices)
            ...
            a_grads = [g for g in a_grads if g is not None]
            ...
            with tf.GradientTape() as ct:
                v_pred = tf.squeeze(self.critic(s_t, training=True))
            ...
            c_grads = [g for g in c_grads if g is not None]
